...
RUN /bin/bash -c '([[ "${METEOR_VERSION}" == "latest" ]] && METEOR_VERSION= ); curl -s https://install.meteor.com/?release=${METEOR_VERSION} | sed -e 's#PREFIX="/usr/local"#PREFIX="/home/ubuntu/.local"#g' | sh'
